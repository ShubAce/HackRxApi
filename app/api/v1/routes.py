...
@router.post(
    ...
    # The dependency is correctly placed here in the decorator
    dependencies=[Depends(verify_token)]
)
async def run_submission(fastapi_request: Request, request: QueryRequest):
    """
    This endpoint combines intelligent caching with a one-shot LLM call for maximum speed.
    1. Caching: Skips the slow ingestion step for previously seen document URLs.
    2. One-Shot Generation: Answers all questions in a single, powerful API call to the LLM.
    """
    ...
    try:
        ...
        for question in request.questions:
            ...
            retrieved_matches = vector_db_service.query(
                ...
                top_k=5,  # Keep this lean for a smaller, faster context
                namespace=namespace
            )
